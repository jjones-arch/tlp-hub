...
const selectCls = "rounded border border-border bg-surface px-2 py-1.5 text-[13px] text-text focus:outline-none focus:ring-1 focus:ring-accent";
...
export default function SboDetailPage() {
  ...
  const [newTask, setNewTask] = useState({ text: "", owner: "", due: "", endDate: "", status: "not-started", priority: "medium" });
  const [editTask, setEditTask] = useState<SboTask | null>(null);
  const [editTaskForm, setEditTaskForm] = useState({ text: "", owner: "", due: "", endDate: "", status: "", priority: "" });
  ...
  async function saveNotes() {
    try { await patchSbo({ notes: notesValue }); toast("Notes saved."); }
  ...
  async function changeStatus(status: string) {
    ...
    try { await patchSbo({ status }); }
    catch { toast("Failed to update status", "err"); }
  }
  ...
  async function addTask() {
    ...
    try {
      ...
      await fetchData();
    } catch { toast("Failed to add task", "err"); }
  }
  ...
  async function saveEditTask() {
    ...
    try {
      ...
      setEditTask(null);
      await fetchData();
    } catch { toast("Failed to update task", "err"); }
  }
  ...
  async function deleteTask(tid: string) {
    try {
      ...
      toast("Task deleted.");
      await fetchData();
    } catch { toast("Failed to delete task", "err"); }
  }
  ...
  async function addMeeting(form: MeetingForm) {
    try {
      ...
      await fetchData();
    } catch { toast("Failed to add meeting", "err"); }
  }
  ...
  async function saveEditMeeting(form: MeetingForm) {
    ...
    try {
      ...
      setEditMeeting(null);
      await fetchData();
    } catch { toast("Failed to update meeting", "err"); }
  }
  ...
  async function deleteMeeting(mid: string) {
    try {
      ...
      await fetchData();
    } catch { toast("Failed to delete meeting", "err"); }
  }
      ...
      <div className="max-w-[1100px] mx-auto px-9 py-10">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-[2fr_1fr] gap-6 mt-8">
            <div className="space-y-4"><div className="h-40 rounded-lg bg-border" /><div className="h-56 rounded-lg bg-border" /></div>
            <div className="space-y-4"><div className="h-40 rounded-lg bg-border" /><div className="h-32 rounded-lg bg-border" /></div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-[1100px] mx-auto px-9 py-10">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4 text-[13px] text-text-2">
          ...
          {data.division && (
            <span className="text-[10.5px] font-semibold uppercase tracking-wide bg-surface-2 px-2 py-0.5 rounded">{data.division}</span>
          )}
        </div>
        {data.description && (
          <p className="text-[13.5px] text-text-2 mt-2 leading-relaxed">{data.description}</p>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowAddTask(true)} className={btnPrimary}>+ Add Task</button>
          <button onClick={() => setShowAddMeeting(true)} className={btnPrimary}>+ Add Meeting</button>
        </div>
      </div>
          ...
          <div className="space-y-6">
            <div className="bg-surface border border-border rounded-lg p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[12px] text-text-3">{tasksDone} of {data.tasks.length} done</span>
              </div>

              {showAddTask && (
                <div className="mb-4 p-3 rounded border border-border bg-surface-2 space-y-2">
                  <input placeholder="Task description" value={newTask.text} onChange={(e) => setNewTask({ ...newTask, text: e.target.value })} className={inputCls} />
                  <div className="grid grid-cols-2 gap-2">
                    <input placeholder="Owner" value={newTask.owner} onChange={(e) => setNewTask({ ...newTask, owner: e.target.value })} className={inputCls} />
                    <input type="date" placeholder="Due" value={newTask.due} onChange={(e) => setNewTask({ ...newTask, due: e.target.value })} className={inputCls} />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <input type="date" placeholder="End date" value={newTask.endDate} onChange={(e) => setNewTask({ ...newTask, endDate: e.target.value })} className={inputCls} />
                    <select value={newTask.status} onChange={(e) => setNewTask({ ...newTask, status: e.target.value })} className={selectCls}>
                      {TASK_STATUSES.map((s) => <option key={s} value={s}>{taskStatusLabel(s)}</option>)}
                    </select>
                    <select value={newTask.priority} onChange={(e) => setNewTask({ ...newTask, priority: e.target.value })} className={selectCls}>
              ...
              {data.tasks.length === 0 ? (
                <p className="text-[13px] text-text-3 py-4 text-center">No tasks yet. Click &ldquo;+ Add Task&rdquo; to add one.</p>
              ) : (
                <ul className="space-y-1">
                  {data.tasks.sort((a, b) => a.sortOrder - b.sortOrder).map((task) => (
                    <li key={task.id} className="group flex items-center gap-2.5 rounded px-2 py-1.5 -mx-2 hover:bg-surface-2 transition-colors">
                      <span className={`w-[16px] h-[16px] rounded-sm border-2 flex-shrink-0 flex items-center justify-center ${
                      ...
                      <span className={`flex-1 text-[13px] ${task.status === "complete" ? "line-through text-text-3" : "text-text"}`}>{task.text}</span>
                      {task.owner && <span className="text-[11px] text-text-3">{task.owner}</span>}
                      <span className={`text-[11px] ${taskStatusClasses(task.status)}`}>{taskStatusLabel(task.status)}</span>
                      ...
                      <div className="opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity">
                        <button onClick={() => { setEditTask(task); setEditTaskForm({ text: task.text, owner: task.owner, due: task.due, endDate: task.endDate, status: task.status, priority: task.priority }); }} className="text-text-3 hover:text-text text-[13px]">✎</button>
                        <button onClick={() => deleteTask(task.id)} className="text-text-3 hover:text-red text-[13px]">✕</button>
                      </div>
                    </li>
                  ))}
                </ul>
      ...
      >
        <div className="space-y-3">
          <div>
            ...
            <input value={editTaskForm.text} onChange={(e) => setEditTaskForm({ ...editTaskForm, text: e.target.value })} className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-text-2 mb-1">Owner</label>
              <input value={editTaskForm.owner} onChange={(e) => setEditTaskForm({ ...editTaskForm, owner: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-text-2 mb-1">Due Date</label>
              <input type="date" value={editTaskForm.due} onChange={(e) => setEditTaskForm({ ...editTaskForm, due: e.target.value })} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-text-2 mb-1">End Date</label>
              <input type="date" value={editTaskForm.endDate} onChange={(e) => setEditTaskForm({ ...editTaskForm, endDate: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-text-2 mb-1">Status</label>
              <select value={editTaskForm.status} onChange={(e) => setEditTaskForm({ ...editTaskForm, status: e.target.value })} className={selectCls + " w-full"}>
                {TASK_STATUSES.map((s) => <option key={s} value={s}>{taskStatusLabel(s)}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[12px] font-medium text-text-2 mb-1">Priority</label>
              <select value={editTaskForm.priority} onChange={(e) => setEditTaskForm({ ...editTaskForm, priority: e.target.value })} className={selectCls + " w-full"}>
                {PRIORITIES.map((p) => <option key={p} value={p}>{p.charAt(0).toUpperCase() + p.slice(1)}</option>)}
              </select>
            </div>
          </div>
