...
export function SortableObjective({
  ...
  const interactive = !!(onToggle && onEdit && onDelete);
  ...
  return (
    <li ref={interactive ? setNodeRef : undefined} style={style} className="flex items-start gap-2.5 group">
      {interactive && (
        <button
          {...attributes}
          {...listeners}
      ...
      {onToggle ? (
        ...
        <span
          className={`mt-0.5 w-[18px] h-[18px] rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
            obj.complete ? "bg-green border-green text-white" : "border-border"
          }`}
        >
          {obj.complete && (
            <svg width="10" height="10" viewBox="0 0 12 12" fill="none">
